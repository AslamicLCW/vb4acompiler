...
public class ArraysTest extends TestCase {
  ...
  /**
   * Tests {@link Arrays#Split(String, String, int)}.
   */
  public void testSplit() {
    ...
    // be thrown.
    try {
      Arrays.Split("", "", Integer.MIN_VALUE);
      fail();
    ...
    String[] result = Arrays.Split("", "", 1); 
    assertEquals(1, result.length);
    assertEquals("", result[0]);

    result = Arrays.Split("", "", Integer.MAX_VALUE);
    assertEquals(1, result.length);
    assertEquals("", result[0]);

    result = Arrays.Split("", "foo", 1);
    assertEquals(1, result.length);
    assertEquals("", result[0]);

    result = Arrays.Split("", "foo", Integer.MAX_VALUE);
    assertEquals(1, result.length);
    assertEquals("", result[0]);
    ...
    result = Arrays.Split("foobarfoobarfoo", "foo", 1);
    assertEquals(1, result.length);
    assertEquals("foobarfoobarfoo", result[0]);

    result = Arrays.Split("foobarfoobarfoo", "foo", 3);
    assertEquals(3, result.length);
    assertContentsInOrder(java.util.Arrays.asList(result), "", "bar", "barfoo");
  
    result = Arrays.Split("foobarfoobarfoo", "foo", Integer.MAX_VALUE);
    assertEquals(4, result.length);
    assertContentsInOrder(java.util.Arrays.asList(result), "", "bar", "bar", "");

    result = Arrays.Split("barfoobarfoobar", "foo", 1);
    assertEquals(1, result.length);
    assertEquals("barfoobarfoobar", result[0]);

    result = Arrays.Split("barfoobarfoobar", "foo", 2);
    assertEquals(2, result.length);
    assertContentsInOrder(java.util.Arrays.asList(result), "bar", "barfoobar");
  
    result = Arrays.Split("barfoobarfoobar", "foo", Integer.MAX_VALUE);
    assertEquals(3, result.length);
